...
import { Facebook, Twitter, Instagram, Linkedin, Github, Mail, ArrowRight, MapPin, Phone, CheckCircle } from 'lucide-react';
...
const Footer = ({ theme }) => {
  ...
  const handleSubscribe = () => {
    // Here you would typically handle the subscription logic with your backend
    setIsSubscribed(true);
    // Optional: Reset after some time to allow subscribing again
            ...
            <div className="flex-1 max-w-lg">
              ...
              <div className="flex flex-col space-y-3">
                <div className="relative">
                  <Mail className={`absolute left-3 top-1/2 -translate-y-1/2 ${themeColors.textMuted}`} size={18} />
                  <input 
                    type="email" 
                    placeholder="Enter your email" 
                    className={`${themeColors.inputBg} ${themeColors.text} w-full px-4 py-2.5 pl-10 rounded-lg border ${themeColors.inputBorder} focus:outline-none focus:ring-1 ${themeColors.ring}`}
                  />
                </div>
                <AnimatedSubscribeButton 
                  subscribeStatus={isSubscribed}
                  onClick={handleSubscribe}
                  className={`${isDark ? 'shadow-blue-900/30' : 'shadow-blue-500/30'} group`}
                >
